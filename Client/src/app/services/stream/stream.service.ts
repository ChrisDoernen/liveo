...
export class StreamService {
    ...
    public getStream(id: string): Promise<Stream> {
        return this._httpClient
            .get(this._endpointService.getEndpoint(`streams/${id}`), { observe: "response", responseType: "json" })
            .pipe(map((response: any) => (response.status === 200) ? response.body as Stream : null))
            .toPromise();
    }
}
