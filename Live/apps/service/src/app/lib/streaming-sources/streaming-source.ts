...
export class StreamingSource implements IStreamingSource {
  ...
  public startStreaming(): void {
    ...
    this._childProcess.on("error", (error) => this._logger.error(`Error spawning child process for device ${this._device.id}: ${error}.`));
    ...
    this._childProcess.on("close", (code) => this._logger.info(`Child process for device ${this._device.id} exited code: ${code}.`));
  }
