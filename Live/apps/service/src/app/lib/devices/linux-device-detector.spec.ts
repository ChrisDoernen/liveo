...
describe('LinuxDeviceDetector', () => {
  ...
  it('should parse devices correctly when two devices are available', () => {
    ...
    promise.then(() => {
      ...
      expect(devices[0].data.description).toBe(
        'ICH5 [Intel ICH5], device 0: Intel ICH [Intel ICH5]'
      );
      expect(devices[1].id).toBe('1');
      expect(devices[1].data.description).toBe(
        'U0x46d0x809 [USB Device 0x46d:0x809], device 0: USB Audio [USB Audio]'
      );
    });
  });
});
